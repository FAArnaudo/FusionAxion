    ...
    public class ControllerFusion
    {
        ...
        public void ActualizarSurtidores()
        {
            foreach (Surtidor surtidor in Station.Instance.Surtidores)
            {
                ...
                foreach (Manguera manguera in surtidor.Mangueras)
                {
                    ...
                    {
                        throw new Exception($"Error al actualizar los surtidores. Excepción: {ex.Message}");
                    }
                }
            }
            Log.Instance.WriteLog("\n", LogType.t_info);
        ...
        public void ActualizarProductos()
        {
            foreach (Producto producto in Station.Instance.Productos)
            {
                ...
                {
                    throw new Exception($"Error al actualizar productos. Excepción: {ex.Message}");
                }
            }
            Log.Instance.WriteLog("\n", LogType.t_info);
        }
            ...
            foreach (Tanque tanque in Station.Instance.Tanques)
            {
                try
                {
                    if (tanque.ID > 0)
                    {
                        ...
                        string rows = string.Format("{0},{1},{2},'{3}'",
                                                     tanque.ID,
                                                     fusionTank.GetFuelVolume(),
                                                     fusionTank.TankVolumeCapacity(),
                ...
                {
                    Log.Instance.WriteLog($"Error al actualizar el tanque({tanque.ID}). Excepción: {e.Message}", LogType.t_error);
                }
            }
            Log.Instance.WriteLog($"\n", LogType.t_info);
        }
